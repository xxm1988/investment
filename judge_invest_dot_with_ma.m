...
label3=judge_ma_rule3(t, price,days_average,2,0.09);
...
label8=judge_ma_rule8(t, price,days_average,5,0.02,0.08);
...
if (price(t)/days_average(t)>1.05)%股价偏离均线程度过大时，不进行操作
    label=0;
elseif(price(t)/days_average(t)<0.95)%股价偏离均线程度过大时，不进行操作
    label=-0;
end
if (label==1||label==-1)
